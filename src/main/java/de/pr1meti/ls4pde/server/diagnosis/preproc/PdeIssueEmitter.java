...
/**
 * ANTLR error listener to inform a preprocess issue listener when syntax errors
 * are encountered.
 *
 * <p>
 * A {BaseErrorListener} which looks for syntax errors reported by ANTLR and
 * converts them to {PdePreprocessIssue}s that are consumable by a
 * {PdePreprocessIssueListener}. It does this by running the
 * {PreprocessIssueMessageSimplifier} to generate a more user-friendly error
 * message before informing the provided listener.
 * </p>
 */
public class PdeIssueEmitter extends BaseErrorListener {
     ...
     *
     * <p>
     * Create a new issue emitter when access to the processing sketch source is not
     * available. Note that this will not allow some error beautification and, if
     * sketch source is available, use other constructor.
     * </p>
     *
     ...
     * Create a new issue emitter.
     *
     * @param newListener      The listener to inform when encountering a syntax
     *                         error.
     * @param newSourceEmitter The sketch source to use when helping beautify
     *                         certain syntax error messages.
     */
    ...
    public void syntaxError(Recognizer<?, ?> recognizer, Object offendingSymbol, int line, int charPositionInLine,
        ...
        if (sourceMaybe.isPresent()) {
            issueLocation = IssueLocationFactory.getLineWithOffset(simplification, line, charPositionInLine,
                    sourceMaybe.get().getSource());
        } else {
            issueLocation = new IssueLocation(line, charPositionInLine);
        }

        listener.onIssue(new PdePreprocessIssue(issueLocation.getLine(), issueLocation.getCharPosition(),
                simplification.getMessage()));
    }

    /**
     * Simple interface for strategy which can emit the full body of a processing
     * sketch.
     */
     ...
     *
     * <p>
     * Data structure describing an edit that was made to an error message or
     * warning to be shown to the user based on a series of rules that attempts to
     * make error messages easier to understand for the user.
     * </p>
     */
    public static class IssueMessageSimplification {
         ...
         *
         * <p>
         * Create a new issue message simplification that leaves the token attribution
         * alone (the token on which the error was reported will be the same before
         ...
         *
         * <p>
         * Create a new issue message simplification. Note that there is an option to
         * have the error attributed to the "prior token". This is helpful, for example,
         * when a semicolon is missing. The error is generated on the token after the
         * line on which the semicolon was omitted so, while the error technically
         * emerges on the next line, it is better for the user for it to appear earlier.
         * Specifically, it is most sensible for it to appear on the "prior token".
         * </p>
         *
         * @param newMessage               The message to show to the user.
         * @param newAttributeToPriorToken Boolean flag indicating if the error should
         *                                 be shown on the token prior to the one on
         *                                 which the error was originally generated.
         *                                 True if the error should be attributed to the
         *                                 prior token. False otherwise.
         */
         ...
         *
         * @return True if the error should be attributed to the prior non-skip token
         *         (not whitepsace or comment). This is useful when a mistake on a prior
         *         line like omitted semicolon causes an error on a later line but one
         *         wants error highlighting closer to the mistake itself. False if the
         *         error should be attributed to the original offending token.
         */
    ...
    public static class IssueLocation {
         ...
         *
         * @param newLine         The line (1-indexed) where the issue occurred. This
         *                        should be in the global file generated by the
         *                        preprocessor and not relative to the start of the tab.
         ...
         *
         * @return The line on which this error occurred. Note that this will be
         *         relative to the global file generated by the preprocessor and not
         *         relative to the start of the tab.
         */
        public int getLine() {
            return line;
        }
         ...
         *
         * @return The number of characters including whitespace from the start of the
     ...
     *
     * <p>
     * For some errors, the location of the "mistake" does not appear close to where
     * the actual error is generated. For example, consider omitting a semicolon.
     * Though the "mistake" is arguably on the line on which a semicolon is
     * forgotten, the grammatical error appears in the first non-skip token after
     * the omitted character. This means that the issue shown to the user may be far
     * away from the line they would want to edit. This utility helps determine if
     * an issue requires a new location and, if so, where the location should be.
     * </p>
     */
    public static class IssueLocationFactory {
         ...
         *
         * @param simplification The issue simplification generated from
         *                       {PreprocessIssueMessageSimplifier}.
         * @param originalLine   The original line (1 indexed) on which the issue was
         *                       reported.
         * @param originalOffset The original number of characters from the start of the
         *                       line where the the issue was reported.
         * @param source         The full concatenated source of the sketch being built.
         * @param lineCount      The total
         * @return The new location where the issue should be reported. This may be
         *         identical to the original location if the issue was not moved.
         */
        public static IssueLocation getLineWithOffset(IssueMessageSimplification simplification, int originalLine,
         ...
         * @param endLineExclusive The line up to which code should be returned. Note
         *                         that this is an "exclusive" boundary. Code from this
         *                         line itself will not be included.
         * @return All of the sketch code leading up to but not including the line
         *         given.
         */
