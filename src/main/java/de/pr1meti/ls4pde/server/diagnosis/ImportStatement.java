...
package de.pr1meti.ls4pde.server.diagnosis;
...
public class ImportStatement {
    ...
    /**
     * Full class name of the import with all packages Ends with star for starred
     * imports
     */
     ...
     * @return ImportStatement which imports all package members in a non-static
     *         context using a wildcard.
     */
     ...
     *
     * @param importString The fully qualified name from which an import statement
     *                     should be built. This supports static prepended so both
     *                     "java.util.List" and "static
     *                     org.processing.package.Factory.build" are supported but
     *                     "import java.util.List;" is not. Note that the static
     *                     prepending is required if the import is static.
     ...
     *
     * @param match The regex match from which an import statement should be built.
     *              Can be from IMPORT_REGEX_NO_KEYWORD or IMPORT_REGEX or
     *              equivalent.
     * @return Newly parsed import statement information.
     */
    public static ImportStatement parse(MatchResult match) {
        ...
        // Deal with static member imports whose "className" is actually a "member
        // name".
     ...
     *
     * @return The fully qualified member name including the parent class. This is
     *         "java.util.List" in the case of "import java.util.List". Note that,
     *         in the case of static imports, it will include the member imported so
     *         "org.processing.package.Factory.build" would be returned for "import
     *         static org.processing.package.Factory.build".
     */
     ...
     *
     * @return This is the class name (List) in the case of "import java.util.List"
     *         or "*" in the case of a wildcard. For static imports this will be the
     *         member within the containing class (Factory.build in the case of
     ...
     *
     * @return The package "java.util" from "import java.util.List". Note that, in
     *         the case of wildcards, the wildcard will be in the member name and
     *         not the class name.
     */
     ...
     *
     * @return True if the FQN (fully qualified name) ends in a wildcard and false
     *         otherwise.
     */
     ...
     *
     * @return True if of the form "import static {FQN}" where FQN refers to the
     *         fully qualified name and false otherwise.
     */
     ...
     * @return True of the two ImportStatements refer to the same import target and
     *         false otherwise.
     */
    public boolean isSameAs(ImportStatement is) {
        return packageName.equals(is.packageName) && memberName.equals(is.memberName) && isStatic == is.isStatic;
    }
}
