...
import de.pr1meti.ls4pde.server.diagnosis.preproc.PdePreprocessor.Mode;
import de.pr1meti.ls4pde.util.ImportStatement;
//import processing.app.Preferences;
import de.pr1meti.ls4pde.util.Parse;
import de.pr1meti.ls4pde.util.SourceUtil;
import de.pr1meti.ls4pde.util.TextTransform;

/**
 * ANTLR tree traversal listener that preforms code rewrites as part of sketch
 * preprocessing.
 *
 * <p>
 * ANTLR tree traversal listener that preforms code rewrites as part of sketch
 * preprocessing, turning sketch source into compilable Java code. Note that
 * this emits both the Java source when using javac directly as part of
 * {JavaBuild} as well as {TextTransform.Edit}s when using the JDT via the
...
public class PdeParseTreeListener extends ProcessingBaseListener {
     ...
     * @param newDestinationPackageName The package to which generated code should
     *                                  be assigned (the package to which the sketch
     *                                  code java file should be assigned).
     */
     ...
     *
     * @param codeFolderImports List of imports for sources sitting in the sketch
     *                          code folder. Note that these will be interpreted as
     *                          non-static imports.
     */
     ...
     * names.
     *
     * @param codeFolderImports List of import statement info for sources sitting in
     *                          the sketch code folder.
     */
    ...
    /**
     * Indicate list of imports required for all sketches to be inserted in
     * preprocessing given those imports' fully qualified names.
     *
     * @param coreImports The list of imports required for all sketches. Note that
     *                    these will be interpreted as non-static imports.
     */
     ...
     *
     * <p>
     * Indicate list of imports that are not required for sketch operation but
     * included for the user's convenience regardless.
     * </p>
     *
     ...
     * <p>
     * Indicate list of imports that are not required for sketch operation but
     * included for the user's convenience regardless given those imports' fully
     * qualified names.
     * </p>
     *
     * @param defaultImports The list of imports to include for user convenience.
     *                       Note that these will be interpreted as non-static
     *                       imports.
     */
     ...
     *
     * @param newListener listener to be informed when an issue is encoutnered in
    ...
    public PreprocessorResult getResult() {
        ...
        return new PreprocessorResult(mode, lineOffset, sketchName, allImports, allEdits, sketchWidth, sketchHeight);
    }

    /*
     * ========================================================= === Implementation
     * of the ANTLR 4 listener functions. ===
     * =========================================================
     */
     ...
     * <p>
     * Endpoint for ANTLR to call when finished parsing an import declaration,
     * remvoing those declarations from sketch body so that they can be included in
     * the header.
     * </p>
     *
    ...
    public void exitImportDeclaration(ProcessingParser.ImportDeclarationContext ctx) {
        ...
        if (ctx.getParent() instanceof ProcessingParser.ClassBodyDeclarationContext) {
            pdeParseTreeErrorListenerMaybe.ifPresent((listener) -> {
                ...
                listener.onError(new PdePreprocessIssue(line, charOffset,
     ...
     *
     * <p>
     * Endpoint for ANTLR to call when finished parsing a floating point literal,
     * adding an 'f' at the end to force it float instead of double for API
     * compatability.
     * </p>
     *
     ...
     * <p>
     * Endpoint for ANTLR to call after parsing a static processing sketch,
     * informing this parser that it is operating on a static sketch (no method or
     * class declarations) so that it writes the correct header / footer.
     * </p>
     *
     ...
     * <p>
     * Endpoint for ANTLR to call after parsing a "active" processing sketch,
     * informing this parser that it is operating on an active sketch so that it
     * writes the correct header / footer.
     * </p>
     *
     * @param ctx ANTLR context for the sketch.
     */
     ...
     *
     * <p>
     * Endpoint for ANTLR to call after parsing a method declaration, making any
     * method "public" that has:
     *
     * <ul>
     ...
     *
     * <p>
     * Endpoint for ANTLR to call after parsing a primitive type name, possibly
     * converting that type to a parse function as part of the Processing API.
     * </p>
     *
    ...
    public void exitFunctionWithPrimitiveTypeName(ProcessingParser.FunctionWithPrimitiveTypeNameContext ctx) {
        ...
        if (!fn.equals("color")) {
            fn = "Parse.parse" + fn.substring(0, 1).toUpperCase() + fn.substring(1);
     ...
     *
     * <p>
     * Endpoint for ANTLR to call after parsing a color primitive token, fixing
     * "color type" to be "int" as part of the processing API.
     * </p>
     *
    ...
    public void exitHexColorLiteral(ProcessingParser.HexColorLiteralContext ctx) {
        if (ctx.getText().length() == 7) {
            insertBefore(ctx.start, ctx.getText().toUpperCase().replace("#", "0xFF"));
        } else {
            insertBefore(ctx.start, ctx.getText().toUpperCase().replace("#", "0x"));
        }
    ...
    /*
     * =========================================================== === Helper
     * functions to parse and manage tree listeners ===
     * ===========================================================
     */
     ...
     *
     * <p>
     * The size call will need to be rewritten if it is in global or setup, having
     * it hoisted up to settings body.
     * </p>
     *
    ...
    protected void handleSizeCall(ParserRuleContext ctx) {
        ...
        if (isInGlobal || isInSetup) {
            ...
            if (isSize && argsContext.getChildCount() > 2) {
                ...
                if (Parse.parseInt(sketchWidth, -1) == -1 && !sketchWidth.equals("displayWidth")) {
                    thisRequiresRewrite = false;
                }

                sketchHeight = argsContext.getChild(2).getText();
                if (Parse.parseInt(sketchHeight, -1) == -1 && !sketchHeight.equals("displayHeight")) {
                    thisRequiresRewrite = false;
                }
    ...
    /*
     * =============================================================================
     * ======= === Utility functions to perform code edit operations and generate
     * rewrite info. ===
     * =============================================================================
     * =======
     *
     * Utility functions to generate and perform code edit operations, performing
     * the edit immediately within a ANTLR rewriter but also generating a
     * {TextTransform.Edit} for use with the JDT. Some of these are left protected
     * for subclasses of PdeParseTreeListener access.
     */
    ...
    /**
     * Prepare preface code to wrap sketch code so that it is contained within a
     * proper Java definition.
     *
    ...
    /**
     * Prepare the footer for a sketch (finishes the constructs introduced in header
     * like class def).
     *
    ...
    protected RewriteResult prepareFooter(TokenStreamRewriter footerWriter, int insertPoint) {
        ...
        PrintWriterWithEditGen decoratedWriter = new PrintWriterWithEditGen(footerWriter, resultBuilder, insertPoint,
                false);
     ...
     *
     * @param decoratedWriter he writer though which the comment should be
     *                        introduced.
     ...
     *
     * @param headerWriter  The writer though which the comment should be
     *                      introduced.
     * @param resultBuilder Builder for reporting out results to the caller.
     */
    protected void writePreprocessorComment(PrintWriterWithEditGen headerWriter, RewriteResultBuilder resultBuilder) {
        ...
        String newCode = String.format("/* autogenerated by Processing preprocessor v%s on %s */", VERSION_STR,
                dateStr);
     ...
     *
     * @param headerWriter  The writer though which the imports should be
     *                      introduced.
     * @param resultBuilder Builder for reporting out results to the caller.
     */
     ...
     *
     * @param headerWriter  The writer through which the header should be
     *                      introduced.
     * @param resultBuilder Builder for reporting out results to the caller.
     */
     ...
     *
     * @param classBodyWriter The writer into which the code should be written.
     *                        Should be for class body.
     * @param resultBuilder   Builder for reporting out results to the caller.
     */
    ...
    protected void writeMain(PrintWriterWithEditGen footerWriter, RewriteResultBuilder resultBuilder) {
        ...
        // footerWriter.addCode("\"" + Parse.ARGS_FULL_SCREEN + "\", ");

        // String bgColor = Preferences.get("run.present.bgcolor");
        // footerWriter.addCode("\"" + Parse.ARGS_BGCOLOR + "=" + bgColor + "\", ");

        // if (Preferences.getBoolean("export.application.stop")) {
        // String stopColor = Preferences.get("run.present.stop.color");
        // footerWriter.addCode("\"" + Parse.ARGS_STOP_COLOR + "=" + stopColor + "\",
        // ");
        // } else {
        // footerWriter.addCode("\"" + Parse.ARGS_HIDE_STOP + "\", ");
        // }
        // }
        ...
        footerWriter.addCodeLine(indent3 + "Parse.main(concat(appletArgs, passedArgs));");
        footerWriter.addCodeLine(indent2 + "} else {");
        footerWriter.addCodeLine(indent3 + "Parse.main(appletArgs);");
    ...
    public static interface PdeParseTreeErrorListener {
        ...
        /**
         * Callback to invoke when an issue is encountered while processing a valid PDE
         * parse tree.
         *
     ...
     *
     * <p>
     * Decorator around a {TokenStreamRewriter} which converts input commands into
     * something that the rewriter can understand but also generates edits saved to
     * an input RewriteResultBuilder. Requires a call to finish() after completion
     * of preprocessing.
     * </p>
     */
    public static class PrintWriterWithEditGen {
         ...
         *
         * @param writer                  The writer to which edits should be
         *                                immediately made.
         * @param newRewriteResultBuilder The builder to which edits should be saved.
         * @param newInsertPoint          The point at which new values should be
         *                                inserted.
         * @param newBefore               If true, the values will be inserted before
         *                                the given insert point. If false, will, insert
         *                                after the insertion point.
         */
        public PrintWriterWithEditGen(TokenStreamRewriter writer, RewriteResultBuilder newRewriteResultBuilder,
                int newInsertPoint, boolean newBefore) {
        ...
        public void finish() {
            ...
            if (before) {
                rewriteResultBuilder
                        .addEdit(PdeParseTreeListener.createInsertBefore(insertPoint, insertPoint, newCode, writer));
            } else {
                rewriteResultBuilder.addEdit(PdeParseTreeListener.insertAfter(insertPoint, newCode, writer));
            }
    ...
    /*
     * =========================================================== === Utility
     * functions to generate code edit operations. ===
     * ===========================================================
     */
    ...
    /**
     * Create a set of non-static imports given the fully qualified names (FQNs) for
     * the types to be imported.
     *
     * @param fullyQualifiedNames The fully qualified names of the types to be
     *                            imported. This should be like "java.util.List".
     *                            Supports wildcards.
     ...
     *
     * @param fullyQualifiedName The fully qualified name of the types to be
     *                           imported. This should be like "java.util.List".
     *                           Supports wildcards.
