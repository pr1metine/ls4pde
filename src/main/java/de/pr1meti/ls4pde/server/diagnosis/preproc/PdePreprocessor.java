...
import de.pr1meti.ls4pde.server.diagnosis.SketchException;
...
public class PdePreprocessor {
    ...
    public static enum Mode {
        /**
         * Sketch without draw, setup, or settings functions where code is run as if the
         * body of a method without any enclosing types. This code will not define its
         * enclosing class or method.
         */
        STATIC,

        /**
         * Sketch using draw, setup, and / or settings where the code is run as if
         * defining the body of a class. This code will not define its enclosing class
         * but it will define its enclosing method.
         */
        ACTIVE,

        /**
         * Sketch written like typical Java where the code is run such that it defines
         * the enclosing classes itself.
         */
        JAVA
    }
     ...
     *
     * Create a new builder to help instantiate a preprocessor for a sketch of the
     * given name. Use this builder to configure settings of the preprocessor before
     * building.
     *
     * @param sketchName The name of the sketch for which a preprocessor will be
     *                   built.
     ...
     *
     * Create a new preprocessor that will use the following set of configuration
     * values to process a parse tree. This object can be instantiated by calling
     * {builderFor}.
     *
     * @param newSketchName     The name of the sketch.
     * @param newTabSize        The number of spaces within a tab.
     * @param newIsTesting      Flag indicating if this is running in unit tests
     *                          (true) or in production (false).
     * @param newFactory        The factory to use for building the ANTLR tree
     *                          traversal listener where preprocessing edits should
     *                          be made.
    ...
    public PdePreprocessor(final String newSketchName, final int newTabSize, boolean newIsTesting,
            final ParseTreeListenerFactory newFactory, List<String> newDefaultImports, List<String> newCoreImports,
            Optional<String> newDestinationPackage) {
     ...
     *
     * @param out     The writer into which the preprocessed code should be written.
     *                This is preferred over returning the full string result as
     *                this string may be large.
     ...
     *
     * @param outWriter          The writer into which the preprocessed code should
     *                           be written. This is preferred over returning the
     *                           full string result as this string may be large.
     * @param inProgram          The sketch ("PDE") code.
     * @param codeFolderPackages The packages included by default for the user by
     *                           virtue of those packages being in the code folder.
     * @return Information about the preprocessing operation.
     */
    public PreprocessorResult write(Writer outWriter, String inProgram, Iterable<String> codeFolderPackages)
        ...
        PdeParseTreeListener listener = listenerFactory.build(tokens, sketchName, tabSize, destinationPackage);
        ...
        listener.setTreeErrorListener((x) -> {
            treeIssues.add(x);
        });
        ...
        ParseTree tree;
        {
            ...
            parser.addErrorListener(new PdeIssueEmitter((x) -> {
                preprocessIssues.add(x);
            }, () -> finalInProgram));
     ...
     *
     * @return Imports included by default but not required by processing or the
     *         mode.
     */
     ...
     * ======================== === Type Definitions === ========================
     *
     * The preprocessor has a sizable number of parameters, including those that can
     * modify its internal behavior. These supporting types help initialize this
     * object and provide hooks for behavior modifications.
     */

    /**
     * Builder to help instantiate a PdePreprocessor.
     *
     * The PdePreprocessor includes a number of parameters, including some
     * behavioral parameters that change how the parse tree is processed. This
     * builder helps instantiate this object by providing reasonable defaults for
     * most values but allowing client to (especially modes) to override those
     * defaults.
     */
    public static class PdePreprocessorBuilder {
         ...
         *
         * <p>
         * The set of imports required by processing itself (in java mode) that are made
         * public so that client code (like in modes) can modify and re-use this list.
         * </p>
         */
        public static final String[] BASE_CORE_IMPORTS = { "processing.core.*", "processing.data.*",
                "processing.event.*", "processing.opengl.*" };

        /**
         * The imports provided as a convenience for the user.
         *
         * <p>
         * The imports that are not strictly required by processing sketches but that
         * are included on behalf of the user that are made public so that client code
         * (like in modes) can modify and re-use this list.
         * </p>
         */
        public static final String[] BASE_DEFAULT_IMPORTS = { "java.util.HashMap", "java.util.ArrayList",
                "java.io.File", "java.io.BufferedReader", "java.io.PrintWriter", "java.io.InputStream",
         ...
         *
         * <p>
         * Create a new preprocessor builder which will use default values unless
         * overridden prior to calling build. Note that client code should use
         * {PdePreprocessor.builderFor} instead of this constructor.
         * </p>
         *
         * @param newSketchName The name of the sketch.
         */
         ...
         *
         * @param newIsTesting Flag that, if true, will configure the preprocessor to
         *                     run safely within a unit testing environment.
         * @return This builder for method chaining.
         */
         ...
         *
         * The ANTLR parse tree listener is where the preprocessing edits are generated
         * and some client code (like modes) may need to override some of the
         * preprocessing edit behavior. Specifying this factory allows client code to
         * replace the default PdeParseTreeListener that is used during preprocessing.
         *
         ...
         *
         * @param newDestinationPackage The package to which output code should be
         *                              assigned.
         * @return This builder for method chaining.
         */
        ...
        public PdePreprocessor build() {
            final int effectiveTabSize = tabSize.orElseGet(() -> 4);

            final boolean effectiveIsTesting = isTesting.orElse(false);

            ParseTreeListenerFactory effectiveFactory = parseTreeFactory.orElse(PdeParseTreeListener::new);

            List<String> effectiveDefaultImports = defaultImports.orElseGet(() -> Arrays.asList(BASE_DEFAULT_IMPORTS));

            List<String> effectiveCoreImports = coreImports.orElseGet(() -> Arrays.asList(BASE_CORE_IMPORTS));

            return new PdePreprocessor(sketchName, effectiveTabSize, effectiveIsTesting, effectiveFactory,
                    effectiveDefaultImports, effectiveCoreImports, destinationPackage);
        }
    ...
    /**
     * Utility function to substitute non ascii characters for escaped unicode
     * character sequences.
     *
