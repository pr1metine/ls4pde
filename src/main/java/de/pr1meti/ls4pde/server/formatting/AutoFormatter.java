 ...
 * version of Jason Pell's GPLed "Java Beautifier" class found <a href=
 * "http://web.archive.org/web/20091018220353/http://geocities.com/jasonpell/programs.html">here</a>,
 * which is itself based on code from Van Di-Han Ho from <a href=
 * "http://web.archive.org/web/20091027043013/http://www.geocities.com/~starkville/vancbj_idx.html">here</a>.
 * [Ben Fry, August 2009]
 */
public class AutoFormatter implements Formatter {
    ...
    /**
     * Number of curly brackets entered and not exited, excluding arrays.
     */
    ...
    /**
     * At every {, this has a true pushed if it's a do-while, and a false otherwise.
     * It is then popped at }.
     */
    private Stack<Boolean> doWhileFlags;

    /**
     * At every (, this has a true pushed for if, while, or for, and a false
     * otherwise. Popped at ).
     */
    ...
    private void writeIndentedLine() {
        ...
        if (startFlag) {
            ...
            boolean indentMore = !buf.toString().matches("[\\s\\]\\}\\)]+;")
                    && (buf.charAt(0) != '{' || arrayLevel >= 0) && overflowFlag;
            if (indentMore) {
                tabs++;
                if (arrayIndent > 0)
                    tabs += arrayIndent;
     ...
     * Sets jdoc_flag if at the start of a doc comment. Sends buf to result with
     * proper indents, then clears buf.<br />
    ...
    private void writeIndentedComment() {
        ...
        int firstNonSpace = 0;
        while (buf.charAt(firstNonSpace) == ' ')
            firstNonSpace++;
        if (lookup_com("/**"))
            jdoc_flag = true;
    ...
    /**
     * Sets pos to the position of the next character that is not ' ' in chars. If
    ...
    /**
     * Increments pos, sets EOF if needed, and returns the new chars[pos] or zero if
     * out-of-bounds. Sets lastNonWhitespace if chars[pos] isn't whitespace. Does
     * nothing and returns zero if already at EOF.
     */
     ...
     * &amp;. Will be different if keyword contains regex codes (except *, which is
     * fine).
     */
            ...
            switch (c) {
                ...
                case '{':
                    ...
                    if (s_if_lev.length == curlyLvl) {
                        s_if_lev = Expand.expand(s_if_lev);
                        s_if_flg = Expand.expand(s_if_flg);
                    }
    ...
    /**
     * Make minor regex-based find / replace changes to execute simple fixes to
     * limited artifacts.
     *
