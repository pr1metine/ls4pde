...
import de.pr1meti.ls4pde.server.diagnosis.preproc.PdePreprocessor;
import de.pr1meti.ls4pde.util.TextTransform.Edit;

public class SourceUtil {

    public static final Pattern IMPORT_REGEX = Pattern.compile(
            "(?:^|;)\\s*(import\\s+(?:(static)\\s+)?((?:\\w+\\s*\\.)*)\\s*(\\S+)\\s*;)",
            Pattern.MULTILINE | Pattern.DOTALL);

    public static final Pattern IMPORT_REGEX_NO_KEYWORD = Pattern
            .compile("^\\s*((?:(static)\\s+)?((?:\\w+\\s*\\.)*)\\s*(\\S+))", Pattern.MULTILINE | Pattern.DOTALL);
    ...
    public static final Pattern TYPE_CONSTRUCTOR_REGEX = Pattern
            .compile("(?<=^|\\W)(int|char|float|boolean|byte)(?=\\s*\\()", Pattern.MULTILINE);
    ...
    // Verifies that whole input String is floating point literal. Can't be used for
    // searching.
    ...
    static {
        ...
        FLOATING_POINT_LITERAL_VERIFIER = Pattern.compile("(?:^" + DIGITS + "\\." + DIGITS + "?" + EXPONENT_PART
                + "?[fFdD]?$)|" + "(?:^\\." + DIGITS + EXPONENT_PART + "?[fFdD]?$)|" + "(?:^" + DIGITS + EXPONENT_PART
                + "[fFdD]?$)|" + "(?:^" + DIGITS + EXPONENT_PART + "?[fFdD]$)");
    }

    // Mask to quickly resolve whether there are any access modifiers present
    private static final int ACCESS_MODIFIERS_MASK = Modifier.PUBLIC | Modifier.PRIVATE | Modifier.PROTECTED;
    ...
    public static final Pattern COLOR_TYPE_REGEX = Pattern.compile(
            "(?:^|^\\p{javaJavaIdentifierPart})(color)\\s(?!\\s*\\()",
    ...
    public static List<Edit> fixFloatsRegex(CharSequence source) {
        ...
        while (matcher.find()) {
            ...
            boolean hasSuffix = end < source.length() && Character.toLowerCase(source.charAt(end)) != 'f'
    ...
    /**
     * Checks a single code fragment (such as a tab) for non-matching braces. Broken
     * out to allow easy use in JavaBuild.
     * 
     * @param c     Program code scrubbed of comments and string literals.
     * @param start Start index, inclusive.
     * @param end   End index, exclusive.
     * @return {@code int[4]} Depth at which the loop stopped, followed by the line
     *         number, column, and string index (within the range) at which an error
     *         was found, if any.
     */
